...
void setup() {
  ...
  Serial.println("[USB INSTALL RESULT] " + String(err));
  ...
  Serial.println("[CDC ACM INSTALL RESULT] " + String(err));
...
void loop() {
  ...
  while (true) {
    ...
    Serial.println(" result " + String(vcp->line_coding_set(&line_coding)));
    ...
    while (!disconnected) {
      ...
      if (ch != 0xFF) {
        if (ch >= ' ' || ch == '\n') {  // character read if OK
          ...
          if (err) Serial.printf("vcp->tx_blocking %d\n", err);
          err = vcp->set_control_line_state(true, true);
          if (err) Serial.printf("vcp->set_control_line_state %d\n", err);
        }
      }
    }
